...
public class HttpControllerTest {
    ...
    @GetMapping("/http/get")
    public String getTest(Member member) { //요청에서 날라온 값들을 Member객체에 담아 주는 역활은 springBoot의 MessageConverter이 해준다.
        log.info("get 테스트");
        log.info(member.toString());
        Member tmpMember = new Member(9, "심상돈", "donbq", "donbq@hanmail.net");
        log.info("tmpMember :  " + tmpMember.toString());
        return "get 요청( id : " + member.getId() + ", username : " + member.getUsername() + ", password : " + member.getPassword() + ", email : " + member.getEmail() + " )";
    }

    //postman에 body에서 x-www-form-urlencoded 방식은 html에서 <form></form>테그를 사용하는것과 같다. 파라메터는 get과 같이 Member member 적어도 무방하다. 이방식을 사용할때는 파라메터를 @RequestBody Member member <== 이게 아닌, Member member 이렇게 선언한다.
    ...
    public String postTest(@RequestBody Member member) { //요청에서 날라온 값들을 Member객체에 담아 주는 역활은 springBoot의 MessageConverter이 해준다.
        log.info("post 테스트");
        //log.info("text : " + text);
        log.info(member.toString());
